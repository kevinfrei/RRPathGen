...
public class Import {
    ...
    private final Pattern trajectoryPattern = Pattern.compile(
        "(\\w+)\\s*\\=\\s*(?:\\s*\\w+(.trajectory(?:Sequence)?Builder))(?:\\s*\\(\\s*)((.|\\r\\n|\\r|\\n)*?)(?=\\.build\\(\\)\\;)"
    );
    private final Pattern markerPattern = Pattern.compile(
        "(?:\\.(UNSTABLE_addTemporalMarkerOffset)\\s*\\(((?:[+-]?(?:\\d*\\.)?\\d+)+),\\s*\\(\\)\\s*\\-\\>\\s*)(?:\\{)((?:.|\\r\\n|\\r|\\n)*?)(?=\\}\\s*\\)\\s*)"
    );
    ...
    public LinkedList<NodeManager> read(File file) {
        ...
        try {
            ...
            while (reader.hasNextLine()) {
                ...
                if (line.matches("^(?!\\s*\\/\\/).*")) allText += line + "\n"; //lines will only be added if they aren't commented
            }
        ...
        for (int i = 0; i < managers.size(); i++) {
            ...
            while (markers.find()) {
                Marker marker = new Marker(
                    Double.parseDouble(markers.group(2)),
                    markers.group(3).trim(),
                    Marker.Type.addTemporalMarker
                );
                manager.add(marker);
            }
        }
