...
public class SettingsPanel extends JPanel {
    ...
    SettingsPanel(Main main, ProgramProperties properties) {
        ...
        for (String label : labels) {
            JTextField input;
            if (Objects.equals(label, labels[3])) input = new JTextField();
            else input = new JFormattedTextField(formatter);
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            field.addActionListener(e -> {
                robot.prop.setProperty(
                    labels[finalI].replaceAll(" ", "_").toUpperCase(),
                    field.getText()
                );
                main.reloadConfig();
