...
public class DrawPanel extends JPanel {
    ...
    public Dimension getPreferredSize() {
        ...
        int width =
            main.getWidth() -
            (main.infoPanel.getWidth() + in.left + in.right + main.exportPanel.getWidth());
    ...
    private void renderRobotPath(
        ...
        BufferedImage image;
        if (this.getWidth() > 0) image = new BufferedImage(
            this.getWidth(),
            this.getHeight(),
            BufferedImage.TYPE_4BYTE_ABGR
        );
        else image = new BufferedImage(1, 1, BufferedImage.TYPE_4BYTE_ABGR);
        Graphics2D g2 = (Graphics2D) image.getGraphics();
        g2.setColor(color);
        double rX = robot.robotLength * main.scale;
        double rY = robot.robotWidth * main.scale;
        double prevHeading = 0;
        if (trajectory.get(0).getDuration() > 0) prevHeading = trajectory.start().getHeading();
        double res;
            ...
            if (segment instanceof TrajectorySegment) {
                ...
                for (double j = 0; j < path.length();) {
                    ...
                    double temp = Math.min(
                        (2 * Math.PI) - Math.abs(pose1.getHeading() - prevHeading),
                        Math.abs(pose1.getHeading() - prevHeading)
                    );
    ...
    public void paintComponent(Graphics g) {
        ...
        main.scale = ((double) this.getWidth() - this.getInsets().left - this.getInsets().right) /
        144.0;
        if (oldScale != main.scale) main
            .getManagers()
            .forEach(nodeManager -> {
        ...
        g.drawImage(
            new ImageIcon(
                Objects.requireNonNull(
                    Main.class.getResource("/field-2024-into-the-deep-juice-dark.jpg")
                )
            ).getImage(),
            0,
            0,
        ...
        if (
            preRenderedSplines == null || preRenderedSplines.getWidth() != this.getWidth()
        ) renderBackgroundSplines();
    ...
    private TrajectorySequence generateTrajectory(NodeManager manager, Node exclude) {
        ...
        TrajectorySequenceBuilder builder = new TrajectorySequenceBuilder(
            new Pose2d(node.x, node.y, Math.toRadians(-node.robotHeading - 90)),
            Math.toRadians(-node.splineHeading - 90),
            new MecanumVelocityConstraint(robot.maxVelo, robot.trackWidth),
            new ProfileAccelerationConstraint(robot.maxAccel),
            Math.toRadians(robot.maxAngVelo),
    ...
    public void renderBackgroundSplines() {
        if (this.getWidth() > 0) preRenderedSplines = new BufferedImage(
            (this.getWidth()),
            this.getHeight(),
            BufferedImage.TYPE_4BYTE_ABGR
        );
    ...
    private void renderArrows(
        ...
        BufferedImage bufferedImage = new BufferedImage(
            preRenderedSplines.getWidth(),
            preRenderedSplines.getHeight(),
            BufferedImage.TYPE_4BYTE_ABGR
        );
        ...
        for (Node node : nodes) {
            ...
            if (!node.reversed) tx.rotate(Math.toRadians(-node.robotHeading + 180));
            else tx.rotate(Math.toRadians(-node.robotHeading));
    ...
    private void mPressed(MouseEvent e) {
        ...
        if (SwingUtilities.isRightMouseButton(e)) {
            ...
            if (closest.distanceToMouse >= (clickSize * main.scale)) closest.index = -1;
